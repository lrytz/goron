...
abstract class ClosureOptimizer {
  ...
  /** If a closure is allocated and invoked within the same method, re-write the invocation to the closure body method.
    *
    * Note that the closure body method (generated by delambdafy:method) takes additional parameters for the values
    * captured by the closure. The bytecode is transformed from
    *
    * [generate captured values] [closure init, capturing values] [...] [load closure object] [generate closure
    * invocation arguments] [invoke closure.apply]
    *
    * to
    *
    * [generate captured values] [store captured values into new locals] [load the captured values from locals] // a
    * future optimization will eliminate the closure [closure init, capturing values] // instantiation if the closure
    * object becomes unused [...] [load closure object] [generate closure invocation arguments] [store argument values
    * into new locals] [drop the closure object] [load captured values from locals] [load argument values from locals]
    * [invoke the closure body method]
    *
    * @param methods
    *   The methods to check for rewrites. If not defined, check all methods with closure instantiations.
    * @return
    *   The changed methods. The order of the resulting sequence is deterministic.
    */
  def rewriteClosureApplyInvocations(
    ...
    val toRewrite =
      mutable.TreeMap.empty[ClosureInstantiation, mutable.ArrayBuffer[(MethodInsnNode, Int)]](closureInitOrdering)
              ...
              for (init <- closureInits.valuesIterator) closureCallsites(init, prodCons) foreach {
                case Left(warning) =>
                  backendReporting.optimizerWarning(
                    warning.pos,
                    warning.toString,
                    backendReporting.siteString(ownerClass, method.name)
                  )
  ...
  /** Insert instructions to store the values captured by a closure instantiation into local variables, and load the
    * values back to the stack.
    *
    * Returns the list of locals holding those captured values, and a list of locals that should be used at the closure
    * invocation callsite to store the arguments passed to the closure invocation.
    */
  ...
  /** Check whether `invocation` invokes the SAM of the IndyLambda `closureInit`.
    *
    * In addition to a perfect match, we also identify cases where a generic FunctionN is created but the invocation is
    * to a specialized variant apply$sp... Vice-versa, we also allow the case where a specialized FunctionN$sp.. is
    * created but the generic apply is invoked. In these cases, the translation will introduce the necessary box / unbox
    * invocations. Example:
    *
    * val f: Int => Any = (x: Int) => 1 f(10)
    *
    * The IndyLambda creates a specialized `JFunction1$mcII$sp`, whose SAM is `apply$mcII$sp(I)I`. The invocation calls
    * `apply(Object)Object`: the method name and type don't match. We identify these cases, insert the necessary unbox
    * operation for the arguments, and invoke the `$anonfun(I)I` method.
    *
    * Tests in InlinerTest.optimizeSpecializedClosures. In that test, methods t4/t4a/t5/t8 show examples where the
    * parameters have to be unboxed because generic `apply` is called, but the lambda body method takes primitive types.
    * The opposite case is in t9: a the specialized `apply$sp..` is invoked, but the lambda body method takes boxed
    * arguments, so we have to insert boxing operations.
    */
  private def isSamInvocation(
    ...
    if (invocation.getOpcode == INVOKESTATIC) false
      ...
      def isSpecializedVersion(specName: String, nonSpecName: String) = specName.startsWith(
        nonSpecName
  ...
  /** The argument types of the lambda body method may differ in two ways from the argument types of the closure member
    * method that is invoked (and replaced by a call to the body).
    *   - The lambda body method may have more specific types than the invoked closure member, see comment in
    *     [[LambdaMetaFactoryCall.unapply]].
    *   - The invoked closure member might be a specialized variant of the SAM or vice-versa, see comment method
    *     [[isSamInvocation]].
    */
  ...
  private def rewriteClosureApplyInvocation(
    ...
    val invocationStackHeight =
      stackHeight + numCapturedValues - 1 + (if (isNew) 2 else 0) // -1 because the closure is gone
    ...
    val bodyMethod =
      byteCodeRepository.methodNode(lambdaBodyHandle.getOwner, lambdaBodyHandle.getName, lambdaBodyHandle.getDesc)
    ...
    val callee = bodyMethod.map({ case (bodyMethodNode, bodyMethodDeclClass) =>
      ...
      Callee(
        ...
        samParamTypes =
          callGraph.samParamTypes(bodyMethodNode, Type.getArgumentTypes(bodyMethodNode.desc), bodyDeclClassType),
  ...
  /** Stores the values captured by a closure creation into fresh local variables, and loads the values back onto the
    * stack. Returns the list of locals holding the captured values.
    */
  ...
  /** Insert store operations in front of the `before` instruction to copy stack values into the locals denoted by
    * `localsList`.
    *
  ...
  /** Insert load operations in front of the `before` instruction to copy the local values denoted by `localsList` onto
    * the stack.
    *
  ...
  object LocalsList {
    ...
    /** A list of local variables starting at `firstLocal` that can hold values of the types in the `types` parameter.
      *
      * For example, `fromTypes(3, Array(Int, Long, String))` returns Local(3, intOpOffset) :: Local(4, longOpOffset) ::
      * // note that this local occupies two slots, the next is at 6 Local(6, refOpOffset) :: Nil
      */
  ...
  /** Stores a local variable index the opcode offset required for operating on that variable.
    *
    * The xLOAD / xSTORE opcodes are in the following sequence: I, L, F, D, A, so the offset for a local variable
    * holding a reference (`A`) is 4. See also method `getOpcode` in [[scala.tools.asm.Type]].
    */
