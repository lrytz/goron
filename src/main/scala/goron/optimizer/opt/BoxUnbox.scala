...
abstract class BoxUnbox {
  ...
  /** Eliminate box-unbox pairs within `method`. Such appear commonly after closure elimination:
    *
    * def t2 = { val f = (b: Byte, i: Int) => i + b // no specialized variant for this function type f(1, 2) // invokes
    * the generic `apply` }
    *
    * The closure optimizer re-writes the `apply` call to `anonfun\$adapted` method, which takes boxed arguments. After
    * inlining this method, we get
    *
    * def t2 = { val a = boxByte(1) val b = boxInteger(2) val r = boxInteger(anonfun\$(unboxByte(a), unboxInt(b)))
    * unboxInt(r) }
    *
    * All these box/unbox operations are eliminated here.
    *
    * Implementation: for every box operation, find all consumers of the boxed value, then all producers of these
    * consumers, repeat until reaching a fixpoint. If this results in a set of boxing and unboxing operations, the box
    * can be eliminated.
    *
    * There are two methods for eliminating boxes: M1: If there is a single boxing operation, the boxed value(s) are
    * stored into new local variable(s) at the allocation site. Accesses to the boxed value are re-written to reads /
    ...
    *     - only for immutable boxes: modifications to the escaped box cannot be applied to the local variable(s)
    *       holding the boxed value(s).
    * Restriction:
    ...
    * value(s) on the stack. Store / load operations that previously acted on the box are adapted to handle the boxed
    * type(s). If the box contains multiple values (or a size-2 value, which doesn't fit into locals that were used for
    * the box), new local slots are used for store / load operations. Restrictions:
    ...
    *
    * E1: M1 only works if there's a single boxing operation. def e1(b: Boolean) = { val i: Integer = box(10) // 10 is
    * stored into a new local, box operation and i removed val j: Integer = box(20) // 20 is stored into a new local,
    * box operation and j removed val r = if (b) i else j // loads and stores of the box are eliminated, r no longer
    * exists unbox(r) // cannot rewrite: we don't know which local to load } Note: the example has no write and the box
    * does not escape, so M2 works here.
    *
    * E2: mutable boxes with multiple boxing operations cannot be eliminated. M1: see E1 M2: cannot replace an `IntRef`
    * on the stack by an `Int` value on the stack, an Int on the stack cannot be modified.
    *
    * def e2(b: Boolean) = { val r1 = new IntRef(0) val r2 = new IntRef(1) val modRef = if (b) r1 else r2 modRef.elem +=
    * 10 // M1: cannot rewrite: which local to write? same as E1. (if (b) r1 else r2).elem += 10 // M2: cannot change an
    * Int on the stack (r1.elem, r2.elem) }
    *
    * E3: escaping boxes with multiple boxing operations cannot be rewritten. M1: see E1. M2: at *, instead of an
    * Integer, an Int is on the stack, but the escape method expects an Integer. We cannot just create a box at this
    * point: if there are multiple escapes (or an escape is executed more than once), the difference could be observed
    * (reference equality).
    *
    * def e3(b: Boolean) = { val i: Integer = box(1) val j: Integer = box(2) escape(if (b) i else j) // * unbox(if (b) i
    * else j) }
    *
    * E4: M1 supports rewriting unbox operations of immutable boxes that escape def e4 = { val i: Integer = box(10) //
    * 10 is stored into a new local, loaded as argument for the box call escape(i) // not changed, still loads the local
    * i holding the box unbox(i) // rewritten to a pop (of the box) and a load of the local variable }
    *
    * E4 seems to be a bit of a corner case, but it's necessary to unblock box eliminations with mutual dependencies.
    * Example:
    *
    ...
    *
    * load 1, load 2, new Tuple2 // stack: Tuple2 load 3 // stack: Tuple2; Int val local1 = new Tuple2 val local2 =
    * local1._1.asInstanceOf[Tuple2] val c = local1._2.asInstanceOf[Int] if (local2 == null) throw new
    * MatchError(local1) val a = local2._1 val b = local2._2 a + b + c
    *
    ...
    *   - there are three consumers of the outer tuple: `local1._1`, `local1._2` and `new MatchError(local1)`. in the
    *     last one, the tuple escapes.
    *   - note that the MatchError creation is dead code: local2 is never null. However, our nullness analysis cannot
    *     identify this: it does not track nullness through tuple stores and loads.
    *   - if we re-write the non-escaping consumers of the outer tuple, but keep the tuple allocation and the escaping
    *     consumer, we get the following:
    *
    * load 1, load 2 val newLocal1 = new Tuple2; load newLocal1 // stack: Tuple2 val newLocal2 = 3; load newLocal2 //
    * stack: Tuple2; Int val local1 = new Tuple2 val local2 = newLocal1 val c = newLocal2 if (local2 == null) throw new
    * MatchError(local1) val a = local2._1 val b = local2._2 a + b + c
    *
    * At this point, the nullness analysis sees that `local2 == null` is false, dead code elimination removes the
    * `throw new MatchError(local1)`. After eliminating the allocation of the outer tuple, the inner tuple (stored in
    * newLocal1) can also be eliminated.
    *
    * Special case for tuples wrt specialization: a tuple getter may box or unbox the value stored in the tuple: calling
    * `_1` on a `Tuple2\$mcII\$sp` boxes the primitive Int stored in the tuple. Similarly, calling `_1\$mcI\$sp` on a
    * non-specialized `Tuple2` unboxes the Integer in the tuple. When eliminating such getters, we have to introduce
    * appropriate box / unbox calls.
    *
    * TODO: add new calls (box / unbox) to the call graph (not urgent) TODO: update the call graph because stack heights
    * change (not urgent). this may also affect other optimizations, we ignored the issue so far. check how stack
    * heights stored in the call graph are used. Note: these tasks are not urgent because the call graph is not
    * currently used during / after method-local optimizations, only before to perform inlining and closure rewriting.
    */
        ...
        def replaceExtractionOps(): Unit = {
          if (boxKind.boxedTypes.lengthCompare(1) == 0) {
            ...
            allConsumers.foreach(extraction =>
              extraction.postExtractionAdaptationOps(boxKind.boxedTypes.head) match {
                case Nil =>
                  extraction match {
              ...
              val replacementOps = extraction match {
                ...
                case _ =>
                  ...
                  if (valueIndex == 0) {
                    ...
                    val consumeStack = boxKind.boxedTypes.zipWithIndex.reverseIterator
                      .map { case (tp, i) =>
                        if (i == valueIndex) {
  ...
  /** Given a box creations operation
    ...
    *
    * Returns a set of box creations and a set of box consumers. Note that the box consumers may contain
    * [[EscapingConsumer]]s, even if there are multiple box creation operations. The callee will handle this case (and
    * not attempt to eliminate the box).
    */
  def allCreationsConsumers(
    ...
    def addBoxConsumers(creation: BoxCreation): Boolean = {
      val newCons =
        creation.boxConsumers(prodCons, ultimate = true).filterNot(cons => consumers.exists(_.consumer == cons))
      newCons.forall(cons =>
  ...
  /** Takes two sets `initialProds` and `finalCons` such that all boxes produced by the first set are only consumed by
    * an operation in the second set.
    *
    * Returns a map that replaces copy operations (ALOAD / ASTORE) between the producers and consumers with
    * corresponding copy operations for the values stored in the box. The returned `Int` value returns the next free
    * local variable slot.
    *
    * Examples:
    *   - for an Integer box, an ASTORE x is simply replaced by ISTORE x
    *   - for a pair of two references, an ASTORE x is replaced by `ASTORE x1; ASTORE x2` where x1 and x2 are fresh
    *     locals
    *
    * Not all copy operations can be supported: DUP only works for single-value boxes, the more exotic copy operations
    * (DUP2_X2) are not supported (note that Scalac never emits them). If a copy operation cannot be replaced, this
    * method returns `None`.
    */
  def checkCopyOpReplacements(
      ...
      nextLocal: Int,
      prodCons: ProdConsAnalyzer
  ): Option[(Map[AbstractInsnNode, List[AbstractInsnNode]], Int, Map[Int, Type])] = {
  ...
  /** For a set of box creation operations and a corresponding set of box consumer operations, this iterator returns all
    * copy operations (load, store, dup) that are in between.
    */
  class CopyOpsIterator(initialCreations: Set[BoxCreation], finalCons: Set[BoxConsumer], prodCons: ProdConsAnalyzer)
      extends AbstractIterator[AbstractInsnNode] {
    private val queue = mutable.Queue.empty[AbstractInsnNode] ++= initialCreations.iterator.flatMap(
  ...
  object BoxKind {
    ...
    /** Check if `newOp` is part of a standard object construction pattern in which:
      ...
      *
      * The method ensures that the entire construction pattern is closed in itself, without any branches going in or
      * out. This is checked by looking at producers / consumers:
      *   - `DUP` is the only consumer of `NEW`, and vice versa
      *   - `DUP` the only producer for the receiver of the constructor call
      *   - The set of consumers of `DUP` without the constructor call is the same as the set of consumers of the value
      *     on the stack top after the constructor call
      */
      ...
      if (newCons.size == 1 && newCons.head.getOpcode == DUP) {
        ...
        if (prodCons.producersForInputsOf(dupOp) == Set(newOp)) {
          ...
          val initCalls = dupCons collect {
            case mi: MethodInsnNode if mi.name == INSTANCE_CONSTRUCTOR_NAME && mi.owner == newOp.desc => mi
          }
          if (initCalls.size == 1) {
    ...
    /** If `mi` is an invocation of a method on Predef, check if the receiver is a GETSTATIC of Predef.MODULE\$ and
      * return it.
      */
  ...
  object PrimitiveBox {
    ...
    private def boxClass(mi: MethodInsnNode) = {
      if (mi.name == INSTANCE_CONSTRUCTOR_NAME) mi.owner
    ...
    def checkPrimitiveBox(
      ...
      insn match {
        ...
        case ti: TypeInsnNode if ti.getOpcode == NEW =>
          for (
            (dupOp, initCall) <- BoxKind.checkInstanceCreation(ti, prodCons) if isPrimitiveBoxConstructor(initCall);
      ...
      insn match {
        case mi: MethodInsnNode =>
          if ((isScalaUnbox(mi) || isJavaUnbox(mi)) && typeOK(mi)) Some(StaticGetterOrInstanceRead(mi))
            ...
            BoxKind.checkReceiverPredefLoad(mi, prodCons).map(ModuleGetter(_, mi))
          else None
  ...
  object Ref {
    ...
    private def loadZeroValue(refZeroCall: MethodInsnNode): List[AbstractInsnNode] = List(
    ...
    def checkRefCreation(
      ...
      insn match {
        ...
        case ti: TypeInsnNode if ti.getOpcode == NEW =>
          for (
            (dupOp, initCall) <- BoxKind.checkInstanceCreation(ti, prodCons) if isRuntimeRefConstructor(initCall);
            kind <- checkKind(initCall)
          )
            yield (InstanceCreation(ti, dupOp, initCall), kind)
    ...
    def checkRefConsumer(insn: AbstractInsnNode, kind: Ref, @unused prodCons: ProdConsAnalyzer): Option[BoxConsumer] =
      insn match {
  ...
  object Tuple {
    ...
    def checkTupleCreation(
      ...
      insn match {
        ...
        case ti: TypeInsnNode if ti.getOpcode == NEW =>
          for (
            (dupOp, initCall) <- BoxKind.checkInstanceCreation(ti, prodCons) if isTupleConstructor(initCall);
            kind <- checkKind(initCall)
          )
            yield (InstanceCreation(ti, dupOp, initCall), kind)
      ...
      insn match {
        case mi: MethodInsnNode =>
          ...
          if (isSpecializedTupleClass(expectedTupleClass)) {
            val typeOK = tupleClass == expectedTupleClass || tupleClass == expectedTupleClass.substring(
              0,
  ...
  sealed trait BoxCreation {
    ...
    /** The consumers of the box produced by this box creation. If `ultimate` is true, then the final consumers are
      * returned (e.g., an unbox operation), otherwise direct consumers (e.g., a store operation).
      */
    def boxConsumers(prodCons: ProdConsAnalyzer, ultimate: Boolean): Set[AbstractInsnNode] = {
      ...
      val cons =
        if (ultimate) prodCons.ultimateConsumersOfOutputsFrom(startInsn) else prodCons.consumersOfOutputsFrom(startInsn)
      this match {
  ...
  case class StaticFactory(producer: MethodInsnNode, loadInitialValues: Option[List[AbstractInsnNode]])
      extends BoxCreation
  ...
  sealed trait BoxConsumer {
    ...
    /** If this box consumer extracts a boxed value and applies a conversion, this method returns equivalent conversion
      * operations. For example, invoking `_1\$mcI\$sp` on a non-specialized `Tuple2` extracts the Integer value and
      * unboxes it.
      */
