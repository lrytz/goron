...
object BytecodeUtils {
  ...
  final val PublicStaticFinal = ACC_PUBLIC | ACC_STATIC | ACC_FINAL
  final val PrivateStaticFinal = ACC_PRIVATE | ACC_STATIC | ACC_FINAL

  // Inlined from scala.reflect.internal.util.Collections
  def foreachWithIndex[A](list: List[A])(f: (A, Int) => Unit): Unit = {
    var i = 0
    var l = list
    while (l.nonEmpty) {
      f(l.head, i)
