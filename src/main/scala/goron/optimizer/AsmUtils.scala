...
object AsmUtils {
  ...
  /** Print the bytecode of methods to the standard output. Only methods whose name contains `traceMethodPattern` are
    * traced.
    */
  final val traceMethodEnabled = false
  final val traceMethodPattern = ""

  /** Print the bytecode of classes to the standard output.
    */
...
//  def main(args: Array[String]): Unit = println(textify(sortedClassRead(classBytes(args.head))))
  ...
  def zapScalaClassAttrs(node: ClassNode): node.type = {
    if (node.visibleAnnotations != null)
      node.visibleAnnotations = node.visibleAnnotations.asScala
        .filterNot(a => a == null || a.desc.contains("Lscala/reflect/ScalaSignature"))
        .asJava
  ...
  /** Run ASM's CheckClassAdapter over a class. Returns None if no problem is found, otherwise Some(msg) with the
    * verifier's error message.
    */
