...
/** Utility for backend components that have state that needs to be re-initialized at every compiler run, for example
  * state that depends on compiler settings of frontend types (Symbols, Types).
  *
