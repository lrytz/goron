...
object InstructionStackEffect {
  ...
  /** Returns the number of stack values consumed and produced by `insn`, encoded in a single `Int` (the `cons` / `prod`
    * extract individual values). The returned values are correct for use in asm's Analyzer framework. For example, a
    * LLOAD instruction produces one stack value. See also doc in `analysis` package object.
    *
    * This method requires the `frame` to be in the state **before** executing / interpreting the `insn`.
    */
  def forAsmAnalysis[V <: Value](insn: AbstractInsnNode, frame: Frame[V]): Int =
    computeConsProd(insn, forClassfile = false, conservative = false, frame = frame)

  /** Returns the maximal possible growth of the stack when executing `insn`. The returned value is usually the same as
    * expected by asm's Analyzer framework, but it may be larger. For example, consider a POP2 instruction:
    *   - if two size-1 values are popped, then the asm Analyzer consumes two values
    *   - if a size-2 value is popped, the asm Analyzer consumes only one stack slot (see doc in the `analysis` package
    *     object)
    *
    * If a precise result is needed, invoke the `forAsmAnalysis` and provide a `frame` value that allows looking up the
    * sizes of values on the stack.
    */
  ...
  /** Returns the number of stack values consumed and produced by `insn`, encoded in a single `Int` (the `cons` / `prod`
    * extract individual values). The returned values are correct for writing into a classfile (see doc on the
  ...
  private def computeConsProd[V <: Value](
    ...
    (insn.getOpcode: @switch) match {
      ...
      case ACONST_NULL | ICONST_M1 | ICONST_0 | ICONST_1 | ICONST_2 | ICONST_3 | ICONST_4 | ICONST_5 | FCONST_0 |
          FCONST_1 | FCONST_2 | BIPUSH | SIPUSH | ILOAD | FLOAD | ALOAD =>
        t(0, 1)
      ...
      case LCONST_0 | LCONST_1 | DCONST_0 | DCONST_1 | LLOAD | DLOAD => if (forClassfile) t(0, 2) else t(0, 1)
      ...
      case IADD | FADD | ISUB | FSUB | IMUL | FMUL | IDIV | FDIV | IREM | FREM => t(2, 1)

      case LADD | DADD | LSUB | DSUB | LMUL | DMUL | LDIV | DDIV | LREM | DREM => if (forClassfile) t(4, 2) else t(2, 1)
      ...
      case IF_ICMPEQ | IF_ICMPNE | IF_ICMPLT | IF_ICMPGE | IF_ICMPGT | IF_ICMPLE | IF_ACMPEQ | IF_ACMPNE => t(2, 0)
      ...
      case IRETURN | FRETURN | ARETURN => t(1, 0) // Frame.execute consumes one stack value
      ...
      case INVOKEVIRTUAL | INVOKESPECIAL | INVOKESTATIC | INVOKEINTERFACE =>
        invokeConsProd(insn.asInstanceOf[MethodInsnNode].desc, insn, forClassfile)
      ...
      case CHECKCAST | INSTANCEOF => t(1, 1) // Frame.execute does push(pop()) for both of them
